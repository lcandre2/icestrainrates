...
saveoutputs = 0; % if 1 = yes, if 0 = no -- Save output .mat files to the current directory for each centered_is2_location
couplinglengthscale = [1,5]; 
coordinatesign = -1;
Cycle = 15; % Change to correct IS2 cycle number based on laser_xy input for filename purposes
Region = 'Pakistoq'; % Change region or glacier name for filename purposes
