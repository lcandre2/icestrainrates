...
% Initialize the figure with Xpi/Ypi
if plotting == 1
    figure
    hold on
    greenlandmap
...
% Need to create an if statement qualifier to make sure this only if above a certain percentage of data (>75%?)
...
for it=1:nt%advecting for nt time-steps
    ...
    %up2 =  [u_interp(centloc), u_interp(topleft), u_interp(topright), u_interp(bottomright), u_interp(bottomleft)];
    %vp2 = [v_interp(centloc), v_interp(topleft), v_interp(topright), v_interp(bottomright), v_interp(bottomleft)];
...
lsqmatrix=[-1/4,1/4,3/4,1/4;0,1/2,0,1/2;3/4,1/4,-1/4,1/4]; %these numbers will change based on orientation of strain diamond
