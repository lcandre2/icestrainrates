...
couplinglengthscale = (1:4);%(1:0.5:5); %determines how many and what lengthscales strain rates are calculated for.
...
largeboundingboxsize = 40; %km, crops the vel and thick data to be more reasonable
...
for i = 1:length(vel)
    ...
    elseif strcmp(velocityerrrorquant, 'std')
        ...
        vel{i,1}.e_vel(:,:,1) = vel{i,1}.e_vel + vel{i,1}.e_vel_std.*numstd;
        vel{i,1}.e_vel(:,:,2) = vel{i,1}.e_vel - vel{i,1}.e_vel_std.*numstd;
        vel{i,1}.n_vel(:,:,1) = vel{i,1}.n_vel + vel{i,1}.n_vel_std.*numstd;
        vel{i,1}.n_vel(:,:,2) = vel{i,1}.n_vel - vel{i,1}.n_vel_std.*numstd;
    ...
    elseif strcmp(velocityerrrorquant, 'montecarlo')
        ...
        size1 = [size(vel{i,1}.x), numsim];
        test  = randn(size1);
        test2 = randn(size1);

        tmpe(:,:,1) = vel{i,1}.e_vel;
        tmpe(:,:,2) = vel{i,1}.e_vel + vel{i,1}.e_vel_std.*numstd;
        tmpe(:,:,3) = vel{i,1}.e_vel - vel{i,1}.e_vel_std.*numstd;
        tmpn(:,:,1) = vel{i,1}.n_vel;
        tmpn(:,:,2) = vel{i,1}.n_vel + vel{i,1}.n_vel_std.*numstd;
        tmpn(:,:,3) = vel{i,1}.n_vel - vel{i,1}.n_vel_std.*numstd;

        edist = test.*vel{i,1}.e_vel_std;
        ndist = test2.*vel{i,1}.n_vel_std; % independent of each other, not true but conservative
        %edist = (tmpe(:,:,2) - tmpe(:,:,3)).*test + tmpe(:,:,3); %scaled randn values to fall within range of +- 1 std
        %ndist = (tmpn(:,:,2) - tmpn(:,:,3)).*test2 + tmpn(:,:,3);

        vel{i,1}.e_vel = vel{i,1}.e_vel_base + edist;
        vel{i,1}.n_vel = vel{i,1}.n_vel_base + ndist;
        %need to keep parent velocity field
        vel{i,1}.e_vel = cat(3,vel{i,1}.e_vel, edist);
        vel{i,1}.n_vel = cat(3, vel{i,1}.n_vel, ndist);
...
elseif velocitydatasource == 2
    dt=vel{i,1}.date_dt;
    nt=(dt/4)-1;% * 4; % Time step for advection and number of steps in days - ITS_LIVE days are variable and large (eg. 435 days)
end
...
for ii = 1:length(is2roi)%:13 %length(is2roi) %This is for each crossover location
    ...
    for jj = 1:numsim+1 %this is for each velocity perturbation
        ...
        u = vel{i,1}.e_vel(:,:,jj);
        v = vel{i,1}.n_vel(:,:,jj); %v = vel.e_vel(:,:,jj);
...
output(i).strainrates                 = strainrates;
output(i).velocityinfo.pixelsize      = vel{i,1}.pixelsize;
output(i).velocityinfo.vel_starttime  = vel{i,1}.start_time;
output(i).velocityinfo.vel_endtime    = vel{i,1}.end_time;
output(i).velocityinfo.e_vel          = vel{i,1}.e_vel;
output(i).velocityinfo.n_vel          = vel{i,1}.n_vel;
output(i).velocityinfo.x_vel          = vel{i,1}.x;
output(i).velocityinfo.y_vel          = vel{i,1}.y;
