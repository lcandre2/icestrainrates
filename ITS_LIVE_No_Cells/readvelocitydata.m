...
%
        ...
        for ii = 1:length(files)
            ...
            [v.x, v.y] = meshgrid(x, y); %these lines make
            v.x = v.x'; v.y =v.y';
            v.x = v.x; v.y = v.y;

            date_center = ncreadatt(files2,'img_pair_info','date_center');
            v.date_center(:,:,ii) = date_center(1:8);
            v.date_dt(:,:,ii) = ncreadatt(files2,'img_pair_info','date_dt');
            v.start_time(:,:,ii) = datetime(v.date_center(1:8),'Format','uuuuMMdd') - (v.date_dt)/2;
            v.end_time(:,:,ii)   = datetime(v.date_center(1:8), 'Format','uuuuMMdd') + (v.date_dt)/2;
           
            
            v.tot_vel(:,:,ii) = ncread(files2,'v');
            v.e_vel(:,:,ii) = ncread(files2,'vx');
            v.n_vel(:,:,ii) = ncread(files2,'vy');
            v.tot_vel_std(:,:,ii) = ncread(files2,'v_error');
            e_vel_std = ncreadatt(files2,'vx','error');
            n_vel_std = ncreadatt(files2,'vy','error');
            v.e_vel_std(:,:,ii) = ones(height(v.x),width(v.y)) * e_vel_std;
            v.n_vel_std(:,:,ii) = ones(height(v.x),width(v.y)) * n_vel_std;
            v.e_vel_base(:,:,ii) = ncread(files2,'vx');
            v.n_vel_base(:,:,ii) = ncread(files2,'vy');
...
v.pixelsize(:,:,ii) = abs(v.x(1) - v.x(2)); %This makes the implicit assumption that the velocity data have 'square' pixels! 
